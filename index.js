...
// توليد OTP
const generateOTP = () => {return crypto.randomInt(100000, 999999);};
...
app.get('/db', (req, res) => {
    ...
    if (db.length === 0) {return res.status(404).json({ message: 'No data found' });}
...
app.post('/forgot-password', async (req, res) => {
    ...
    if (email && db.find(user => user.email === email)) {
        ...
        let forgetPassTemplate = fs.readFileSync(path.join(__dirname, 'forgotPassword.html'), 'utf-8');
...
app.listen(sever_port, () => {
  ...
  process.on('unhandledRejection',err=>{
    // eslint-disable-next-line no-undef
    console.error(`Unhandled Rejection Error: ${err.name} | ${err.message}${(node_env === 'development')&& '\n    '+err.stack}`);
